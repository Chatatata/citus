...
static TimestampTz ParseTimestampTzField(PGresult *result, int rowIndex, int colIndex, bool *isNull);
...
static LockDepNode * LookupDepNode(HTAB *lockDepNodeHash, TmgmtTransactionId *transactionId);
...
this_machine_kills_deadlocks(PG_FUNCTION_ARGS)
{
	 ...
	 */
	{
		...
		while ((curNode = (LockDepNode *) hash_seq_search(&initNodeSeq)) != 0)
		{
			...
			while ((resetVisitedNode = (LockDepNode *) hash_seq_search(&resetVisitedSeq)) != 0)
			{
...
BuildLocalWaitForGraph(int sourceNodeId, int localNodeId)
{
	...
	while (todoList)
	{
		...
		TmgmtBackendData* curBackendData = &TmgmtShmemControl->sessions[curProc->pgprocno];
		...
		if (curProc->links.next != NULL && curProc->waitLock != NULL)
		{
			...
			while (proclock)
			{
				...
				leader = nextProc->lockGroupLeader == NULL ? nextProc : nextProc->lockGroupLeader;
				...
				if (leader != curProc)
				{
					...
					for (lockMethodOff = 1; lockMethodOff <= numLockModes; lockMethodOff++)
					{
						if ((proclock->holdMask & LOCKBIT_ON(lockMethodOff)) &&
							(conflictMask & LOCKBIT_ON(lockMethodOff)))
						{
							bool blockingAlsoBlocked = nextProc->links.next != NULL && nextProc->waitLock != NULL;
...
AllocWaitEdge(WaitGraph *waitGraph)
{
	...
	if (waitGraph->numAllocated == waitGraph->numUsed)
	{
		...
		waitGraph->edges = (WaitEdge *)
			repalloc(waitGraph->edges, sizeof(WaitEdge) * waitGraph->numAllocated);
	}
...
ParseTimestampTzField(PGresult *result, int rowIndex, int colIndex, bool *isNull)
{
	...
	timestampDatum =
		DirectFunctionCall3(timestamptz_in, CStringGetDatum(PQgetvalue(result, rowIndex, colIndex)),
							0, -1);
...
TransactionIdHash(const void *key, Size keysize)
{
	...
	hash = hash_combine(hash, hash_any((unsigned char *) &entry->transactionId, sizeof(int64)));
	hash = hash_combine(hash, hash_any((unsigned char *) &entry->timestamp, sizeof(TimestampTz)));
